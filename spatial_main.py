...
def main(args):
    ...
    if hasattr(train_loader.dataset, 'ethnicity_columns'):
        ...
        if 'train_pop_2001' in data:
            ethnicity_names = [col for col in data['train_pop_2001'].columns if col != 'DAUID']
    ...
    logger.info(f"Population vectors dimension: {sample_batch['population_2001'].shape[1]}")
    ...
    try:
        ...
        if test_loader and any(f"{k}_2006" in data for k in ['test_pop', 'test_pop_2006']):
        ...
        # Generate predictions if test data is available but without 2006 population
        if test_loader and not any(f"{k}_2006" in data for k in ['test_pop', 'test_pop_2006']):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hidden_dim_1_base", type=int, default=27,
                        help="Base size for first hidden dimension")
    parser.add_argument("--hidden_dim_2_base", type=int, default=11,
                        help="Base size for second hidden dimension")
    parser.add_argument("--hidden_dim_3_base", type=int, default=4,
                        help="Base size for third hidden dimension")
    parser.add_argument("--coord_embed_base", type=int, default=7,
                        help="Base size for coordinate embedding dimension")
    parser.add_argument("--dropout_rate", type=float, default=0.11549007998447319,
                        help="Dropout rate")
    parser.add_argument("--colonization_threshold", type=float, default=0.12211010989323141,
                        help="Threshold below which population is considered zero")
    parser.add_argument("--colonization_expert_weight", type=float, default=0.5561910237863912,
                        help="Base weight for colonization expert influence")
    parser.add_argument("--colonization_weight", type=float, default=4.013156586009125,
                        help="Weight multiplier for colonization loss")
    parser.add_argument("--jump_threshold", type=float, default=76.02602681139794,
                        help="Threshold for identifying potential jump cases")
    parser.add_argument("--jump_expert_weight", type=float, default=0.5709098241179948,
                        help="Base weight for jump expert influence")
    parser.add_argument("--jump_weight", type=float, default=8.428747610454728,
                        help="Weight multiplier for jump loss")
    parser.add_argument("--decline_threshold", type=float, default=75.54437608969272,
                        help="Threshold for identifying potential decline cases")
    parser.add_argument("--decline_expert_weight", type=float, default=0.7488347084633729,
                        help="Base weight for decline expert influence")
    parser.add_argument("--decline_weight", type=float, default=6.361538367632025,
                        help="Weight multiplier for decline loss")
    parser.add_argument("--pde_expert_weight", type=float, default=0.529750818372629,
                        help="Base weight for PDE expert influence")
    parser.add_argument("--pde_weight", type=float, default=5.172326697890094,
                        help="Weight multiplier for PDE loss cases")
    ...
    parser.add_argument("--learning_rate", type=float, default=0.0005555268811326451,
                        help="Learning rate")
    parser.add_argument("--weight_decay", type=float, default=0.0005639821283459794,
