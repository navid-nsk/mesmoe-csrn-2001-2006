...
class MultiEthnicPopulationDataset(Dataset):
    ...
    """
    def __init__(self, features_df, population_df_2001, population_df_2006=None, 
        ...
        Args:
            ...
            population_df_2001 (pd.DataFrame): DataFrame containing 2001 multi-ethnic population data
            population_df_2006 (pd.DataFrame, optional): DataFrame containing 2006 multi-ethnic population data
        ...
        """
        # Merge features with 2001 population data on DAUID
        self.data = pd.merge(features_df, population_df_2001, on='DAUID', how='inner')
        
        # Store ethnicities columns (all except DAUID)
        self.ethnicity_columns = [col for col in population_df_2001.columns if col != 'DAUID']
        ...
        # If 2006 population data is provided (for training), merge it as well
        if population_df_2006 is not None:
            # Use suffixes to distinguish between 2001 and 2006 population columns
            self.data = pd.merge(self.data, population_df_2006, on='DAUID', 
                               how='inner', suffixes=('_2001', '_2006'))
        ...
        # Extract 2001 population vectors
        self.population_2001 = self.data[[f"{col}_2001" if population_df_2006 is not None else col 
                                        for col in self.ethnicity_columns]].values
        
        # Extract 2006 population vectors if available
        if population_df_2006 is not None:
            self.population_2006 = self.data[[f"{col}_2006" for col in self.ethnicity_columns]].values
        else:
            self.population_2006 = None
    ...
    def __getitem__(self, idx):
        ...
        pop_2001 = self.population_2001[idx]
        ...
        # If training (2006 data available), return features and target
        if self.population_2006 is not None:
            return {
                'dauid': dauid,
                'features': torch.tensor(features, dtype=torch.float32),
                'population_2001': torch.tensor(pop_2001, dtype=torch.float32),
                'coordinates': torch.tensor(coords, dtype=torch.float32),
                'target': torch.tensor(self.population_2006[idx], dtype=torch.float32)
            }
...
class MultiEthnicDataHandler:
    ...
    def load_data(self):
        ...
        files = {
            'train_features': os.path.join(self.data_dir, '2001_train_features_all.csv'),
            'train_pop_2001': os.path.join(self.data_dir, '2001_train_population_all.csv'),
            'train_pop_2006': os.path.join(self.data_dir, '2006_train_population_all.csv'),
            'val_features': os.path.join(self.data_dir, '2001_val_features_all.csv'),
            'val_pop_2001': os.path.join(self.data_dir, '2001_val_population_all.csv'),
            'val_pop_2006': os.path.join(self.data_dir, '2006_val_population_all.csv'),
            'test_features': os.path.join(self.data_dir, '2001_test_features_all.csv'),
            'test_pop_2001': os.path.join(self.data_dir, '2001_test_population_all.csv'),
        ...
        for key, file_path in files.items():
            if os.path.exists(file_path):
                ...
                if 'test_pop_2006' in key:  # This might be intentionally missing for final testing
    ...
    def create_dataloaders(self, data, batch_size=32):
        ...
        val_dataset = None
        if 'val_features' in data and 'val_pop_2001' in data and 'val_pop_2006' in data:
        ...
        test_dataset = None
        if 'test_features' in data and 'test_pop_2001' in data:
            # For test dataset, we may or may not have 2006 population data
            test_pop_2006 = data.get('test_pop_2006')
